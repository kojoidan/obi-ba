...
const InputField = ({ type, placeholder, name, handleChange, address }) =>(
    ...
    outline-none text-gray-500 focus:border-yellow-600 transition'
     type={type}
...
const AddAddress = () => {
    ...
    const onSubmitHandler = async (e) => {
        e.preventDefault();
        try {
            // map client `zipCode` to backend `zipcode` (Number)
            const payload = {
                ...address,
  ...
  return (
    <div className='mt-16 pb-16'>
        <p className='text-2xl md:text-2xl text-gray-500'>Add Delivering <span 
        className='font-semibold text-yellow-600'>Address</span></p>
        <div className='flex flex-col-reverse md:flex-row justify-between mt-10'>
            <div className='flex-1 max-w-sm'>
                <form onSubmit={onSubmitHandler} className='space-y-3 mt-6 text-sm'>
                    ...
                    <button className='w-full mt-6 bg-yellow-600 py-3 text-white hover:bg-yellow-700
